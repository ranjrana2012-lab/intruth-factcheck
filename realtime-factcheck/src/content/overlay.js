...
function loadSettings() {
  chrome.storage.local.get(['intruthSettings'], (data) => {
    if (data.intruthSettings) settings = { ...DEFAULT_SETTINGS, ...data.intruthSettings };
    applySettings();
  });
}

function saveSettings() {
  chrome.storage.local.set({ intruthSettings: settings });
  applySettings();
}

function applySettings() {
  if (!panel) return;
  panel.dataset.size     = settings.panelSize;
  panel.dataset.fontSize = settings.fontSize;
}

function shouldShowVerdict(verdict) {
  if (settings.verdictFilter === 'all') return true;
  if (settings.verdictFilter === 'flagged') return verdict === 'FALSE' || verdict === 'MISLEADING';
  if (settings.verdictFilter === 'accurate') return verdict === 'TRUE' || verdict === 'SUBSTANTIALLY TRUE';
  return true;
}
...
// mirror service worker window counter to snapshot timestamps at window boundaries
const WINDOW_SIZE_MIRROR = 4;
let overlaySentenceCount = 0;
const windowBoundaryTimestamps = new Map(); // windowIndex → timestamp
...
const seenSpeakers        = new Map(); // { speakerId: sampleText } — for re-firing banners

function showSpeakerBanner(speakerId, sample) {
  const sid = String(speakerId);
  if (sample) seenSpeakers.set(sid, sample);
...
function getSpeakerNameForCard(result) {
  const sid = result.dominantSpeakerId;
  if (sid === null || sid === undefined) return null;
  const confirmed = confirmedSpeakerMap[String(sid)];
  if (confirmed) return normalizeSpeakerName(confirmed);
  return null;
}
function retryTagAllCards() {
  ...
  verdictListEl.querySelectorAll('.rtfc-verdict:not(.rtfc-verdict--pending)').forEach(card => {
    if (!card._resultData) return;
    const name = getSpeakerNameForCard(card._resultData);
    if (!name) return;
...
function renderSpeakerEditor() {
  ...
  el.querySelectorAll('.rtfc-speaker-chip-input').forEach(input => {
    input.addEventListener('change', (e) => {
      ...
      sendSpeakerMap();
      chrome.runtime.sendMessage({ type: 'RESET_SPEAKERS' }).catch(() => {});

      // re-fire banners for ALL seen speakers with updated button names
      // don't skip confirmed ones — user may have clicked wrong name before editing chip
      seenSpeakers.forEach((sample, sid) => {
        panel?.querySelectorAll('.rtfc-speaker-banner').forEach(b => {
          if (b.querySelector(`[data-id="${sid}"]`)) b.remove();
        });
        delete confirmedSpeakerMap[sid];
        pendingSpeakerIds.delete(sid);
        showSpeakerBanner(sid, sample);
      });
...
function createPanel() {
  ...
  panel.innerHTML = [
    '<div id="rtfc-header">',
      ...
      '<div class="rtfc-header-actions">',
        '<button id="rtfc-settings" title="Settings">⚙</button>',
        '<button id="rtfc-export" title="Export session as PDF">↓ Export</button>',
        '<button id="rtfc-close">✕</button>',
      '</div>',
    '</div>',
    '<div id="rtfc-settings-drawer" style="display:none">',
      '<div class="rtfc-setting-row">',
        '<span class="rtfc-setting-label">Panel size</span>',
        '<div class="rtfc-setting-options" data-setting="panelSize">',
      ...
      '</div>',
      '<div class="rtfc-setting-row">',
        '<span class="rtfc-setting-label">Font size</span>',
        '<div class="rtfc-setting-options" data-setting="fontSize">',
          '<button data-value="small">S</button>',
          '<button data-value="normal">M</button>',
          '<button data-value="large">L</button>',
        '</div>',
      '</div>',
      '<div class="rtfc-setting-row">',
        '<span class="rtfc-setting-label">Show verdicts</span>',
        '<div class="rtfc-setting-options" data-setting="verdictFilter">',
          '<button data-value="all">All</button>',
          '<button data-value="flagged">FALSE / MISLEADING</button>',
          '<button data-value="accurate">TRUE / SUBST. TRUE</button>',
        '</div>',
      '</div>',
    '</div>',
    '<div id="rtfc-body">',
  ...
  panel.querySelector('#rtfc-settings').addEventListener('click', () => {
    const drawer = panel.querySelector('#rtfc-settings-drawer');
  ...
  panel.querySelectorAll('.rtfc-setting-options').forEach(group => {
    ...
    group.querySelectorAll('button').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        saveSettings();
        group.querySelectorAll('button').forEach(b => b.classList.toggle('active', b.dataset.value === settings[key]));
        if (key === 'verdictFilter') refilterVerdicts();
      });
    });
  });
...
function removePanel() {
  ...
  overlaySentenceCount = 0;
  windowBoundaryTimestamps.clear();
...
function buildCard(result) {
  ...
  let speakerTag = '';
  if (!result.pending) {
    const name = getSpeakerNameForCard(result);
    if (name) {
      const color = getSpeakerColor(name);
  ...
  card.innerHTML = [
    ...
    '<div class="rtfc-verdict-header">',
      ...
      result._timestamp ? '<span class="rtfc-timestamp">⏱ ' + escapeHtml(result._timestamp) + '</span>' : '',
    '</div>',
  ...
  // click anywhere on card (except links/buttons) to seek video to timestamp
  card.addEventListener('click', (e) => {
    if (e.target.closest('a, button')) return; // don't intercept source links or buttons
    if (result._timestamp) seekVideo(result._timestamp);
  });
  if (result._timestamp) card.style.cursor = 'pointer';
...
function refilterVerdicts() {
  ...
  verdictListEl.querySelectorAll('.rtfc-verdict').forEach(card => {
    const verdict = card._resultData?.verdict || '';
    card.style.display = shouldShowVerdict(verdict) ? '' : 'none';
  });
}
...
function addVerdict(result) {
  ...
  if (!result._timestamp) {
    // use the exact window boundary timestamp matching this verdict's window
    // windowIndex is sent by service worker at evaluation time
    const exactTs = result.windowIndex !== undefined
      ? windowBoundaryTimestamps.get(result.windowIndex)
      : null;
    result._timestamp = exactTs || getClaimTimestamp(result.claim);
  }
  if (result.dominantSpeakerId !== null && result.dominantSpeakerId !== undefined) {
  ...
  if (!shouldShowVerdict(result.verdict)) card.style.display = 'none';
  verdictListEl.prepend(card);
}


function formatBias(rating) {
  const map = {
    'left': 'L', 'left-center': 'LC', 'center': 'C',
    'right-center': 'RC', 'right': 'R',
    'conspiracy': '\u26a0', 'satire': '~',
  };
  return map[rating] || '?';
}

function annotateBias(cardEl) {
  const links = cardEl.querySelectorAll('a[href]');
  links.forEach(link => {
    try {
      const domain = new URL(link.href).hostname.replace(/^www\./, '');
      chrome.runtime.sendMessage({ type: 'GET_BIAS', domain }, (data) => {
        if (chrome.runtime.lastError || !data?.rating) return;
        const pill = document.createElement('span');
        pill.className = 'rtfc-bias-pill rtfc-bias-' + data.rating.replace(/[^a-z]/gi, '-');
        pill.textContent = formatBias(data.rating);
        pill.title = data.rating + (data.source ? ' (' + data.source + ')' : '');
...
function sortVerdictCards() {
  if (!verdictListEl) return;
  const cards = Array.from(verdictListEl.querySelectorAll('.rtfc-verdict'));
  if (cards.length < 2) return;
  cards.sort((a, b) => {
    const ta = a._resultData?._timestamp ?? 0;
    const tb = b._resultData?._timestamp ?? 0;
    return tb - ta; // newest first (prepend order)
  });
  cards.forEach(card => verdictListEl.appendChild(card));
}
...
function updateVerdict(result) {
  ...
  applyVerdictToBullet(result.claim, result.verdict, result.confidence);
  logVerdict(result);
  // annotate sources with bias ratings
  if (newCard) annotateBias(newCard);
  sortVerdictCards();
}
  ...
  switch (msg.type) {
    ...
    case 'TRANSCRIPT_RESULT':
      if (msg.interim) {
        ...
        // snapshot at window boundaries to get accurate claim timestamps
        overlaySentenceCount++;
        if (overlaySentenceCount % WINDOW_SIZE_MIRROR === 0) {
          const wIdx = Math.floor(overlaySentenceCount / WINDOW_SIZE_MIRROR);
          windowBoundaryTimestamps.set(wIdx, ts);
          // keep map bounded
          if (windowBoundaryTimestamps.size > 20) {
            const oldest = Math.min(...windowBoundaryTimestamps.keys());
            windowBoundaryTimestamps.delete(oldest);
          }
        }
        clearInterim();
